...
const retrieveCheckoutFormResult = (checkoutFormRequest) => {
  return new Promise((resolve, reject) => {
    iyzipay.checkoutForm.retrieve(checkoutFormRequest, (error, result) => {
      if (error) {
        reject(error);
        return;
      }
...
const createCheckoutForm = async (req, res) => {
  try {
    ...
    // Bu satır hem inStock'a hem de stock sayısına bakar.
    // İkisinden biri doğruysa geçişe izin verir.
    if (
...
const handleCheckoutCallback = async (req, res) => {
  try {
    ...
    const orderId = checkoutFormResult.basketId || checkoutFormResult.conversationId || conversationId;

    console.log("Iyzico'dan Gelen Ham Sonuç:", checkoutFormResult.status);
    console.log("Bulunan Sipariş ID:", orderId);
    ...
    if (orderId && orderId !== "undefined") {
      ...
      if (updatedOrder) {
        console.log("✅ Veritabanı GÜNCELLENDİ:", updatedOrder.paymentStatus);
      } else {
        console.log("❌ HATA: Bu ID ile veritabanında sipariş bulunamadı!");
      }
    } else {
      console.log("❌ HATA: Sipariş ID hala bulunamadı!");
    }
